...
class Team:
    ...
    def s_change_manual(self):
        m_stat = input(f"{self.id} Zadejte názem nového statu:\n")
        m_value = input(f"{self.id} Zadejte hodnotu statu {m_stat}:\n")
        print(f"{self.id} Stat: {m_stat}, hodnota {m_value}.")
        if self.confirm(): 
            self.edit_stat(m_stat, m_value)

    def stat_del(self, stat):
        try:
            self.stats.pop(stat)
        except:
            print(f"{self.id} Stat {stat} nenalezen.")
...
def grid_generate():
    for i in range(100): #generátor políček
...
#Začátek funkce týmů
with open(color_values, "r", encoding="UTF-8") as soubor:
    colors = {}
    for color in soubor.readlines():
        color = color.strip().split()
        colors[color[0]] = color[1:]

with open(data_teams, "r", encoding="UTF-8") as soubor:
    vstup = []  
    for line in soubor.readlines():
        line = line.strip().split(maxsplit=2)
        vstup.append(line)

with open(data_tiles, "r", encoding="UTF-8") as soubor:
    dilky = []
    for line in soubor.readlines():
        x = line.strip().split()
        if x[1] == "None":
...
with open(data_teams, "w", encoding="UTF-8") as soubor:    
    for team in teams:
        team.spend_money()
        team.export(soubor)
